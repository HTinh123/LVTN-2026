...
function AdvisorList() {
  ...
  // Fetch classes by department for modal
    ...
    try {
      ...
      switch (modalMode) {
        case 'add':
          ...
          method = 'POST';
          body = {
            ms_cvht: formData.ms_cvht,
            hoten: formData.hoten,
            username: formData.username,
            password: formData.password,
            mslop: formData.mslop,
            start_ms_hocky: parseInt(formData.start_ms_hocky),
            end_ms_hocky: parseInt(formData.end_ms_hocky)
          };
          break;
                      ...
                      {modalMode === 'add' && (
                        <div className="mt-4 space-y-3">
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label className="block text-sm font-medium text-gray-700">CVHT ID *</label>
                              <input
                                type="text"
                                required
                          ...
                          <div className="border-t border-gray-200 pt-3">
                            <p className="text-sm font-medium text-gray-700 mb-2">Class Assignment</p>
                            ...
                            {/* Class Dropdown - Disabled if no department selected */}
                            <div className="mb-3">
                              ...
                              <select
                                ...
                                disabled={!formData.ms_khoa}
                                className={`text-black mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1.5 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm ${
                                  !formData.ms_khoa ? 'bg-gray-100 cursor-not-allowed' : ''
                                }`}
                              >
                                <option value="">{formData.ms_khoa ? 'Select Class' : 'Select Department First'}</option>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <div>
                                <label className="block text-sm font-medium text-gray-700">Begin Semester *</label>
                                <select
                                  required
                                  value={formData.start_ms_hocky}
                                  onChange={(e) => setFormData({ ...formData, start_ms_hocky: e.target.value })}
                                  className="text-black mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1.5 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                >
                                  <option value="">Select Begin</option>
                                  {semesters.map(sem => (
                              ...
                              <div>
                                <label className="block text-sm font-medium text-gray-700">End Semester *</label>
                                <select
                                  required
                                  value={formData.end_ms_hocky}
                                  onChange={(e) => setFormData({ ...formData, end_ms_hocky: e.target.value })}
                                  className="text-black mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-1.5 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                >
                ...
                <button
                  ...
                  disabled={
                    (modalMode === 'add' && (
                      ...
                      !formData.ms_khoa ||
                      !formData.mslop ||
                      !formData.start_ms_hocky ||
                      !formData.end_ms_hocky
                    )) ||
